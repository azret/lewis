...
public class XmlNode {
    ...
    public bool Is(XmlNodeType type) {
        return type == _type;
    }
    public bool Is(string name) {
        return string.Equals(name, _name);
    }
    public bool IsNot(string name) {
        return !string.Equals(name, _name);
    }
    XmlNode[] _children;
    ...
    public void Add(XmlNode child) {
        if (_children == null) {
            _children = new XmlNode[0];
        }
        int len = _children.Length;
        Array.Resize(ref _children, len + 1);
        _children[len] = child;
    }
}
...
public class Xml {
                ...
                if (i < ln && text[i] == '#') {
                    ...
                    if (i > start) {
                        ...
                        for (int k = digits.Length - 1; k >= 0; k--) {
                            val = val + ((int)Math.Pow(10, (digits.Length - k - 1))) * (digits[k] - '0');
                        }
                    ...
                    if (i > start) {
                        int end = i; bool isEntity = false;
                        if (i < ln && text[i] == ';') {
                            i++; isEntity = true;
                        }
                        var entity = text.Substring(start, end - start).ToLowerInvariant();
                        string replace;
                        if (Enities.TryGetValue(entity, out replace)) {
                            plain.Append(replace);
                        } else {
                            if (isEntity) {
                                plain.Append($"&{entity};");
    ...
    static void parseAttributes(ref int i, int len, string buffer) {
    ...
    public void read() {
        ...
        int len;
        if (buffer == null || (pos >= (len = buffer.Length))) {
            return;
        }
        int start = pos; char c = buffer[pos++];
        switch (c) {
            case '<': {
                    token = XmlTokenType.Text;
                    if (pos < len && char.IsLetter(buffer[pos])) {
                        token = XmlTokenType.Start;
                        int tagNameStart = pos++;
                        while (pos < len && (char.IsLetterOrDigit(buffer[pos]) || buffer[pos] == '.')) {
                            pos++;
                        }
                        ...
                        while (pos < len && buffer[pos] != '>') {
                            if (buffer[pos] == '/' && (pos + 1 < len) && buffer[pos + 1] == '>') {
                                pos++;
                                token = XmlTokenType.SelfClosing;
                                break;
                            }
                            pos++;
                        }
                    ...
                    } else if (pos < len && buffer[pos] == '?') {
                        pos++;
                        if (pos < len && char.IsLetter(buffer[pos])) {
                            token = XmlTokenType.ProcessingInstruction;
                            int tagNameStart = pos++;
                            while (pos < len && (char.IsLetterOrDigit(buffer[pos]) || buffer[pos] == '.')) {
                                pos++;
                            }
            ...
            default: {
                    ...
                    node = buffer.Substring(start, pos - start);
                }
                break;
        }
    }
    ...
    public void Parse(XmlNode parent) {
        ...
        while (token == XmlTokenType.Start) {
            ...
            while (token == XmlTokenType.Text
                            || token == XmlTokenType.SelfClosing
                                        || token == XmlTokenType.ProcessingInstruction) {
                if (token == XmlTokenType.Text) {
                    child.Add(new XmlNode(XmlNodeType.Text, "#TEXT", praseText(this.node)));
                }
...
static class App {
    static void Main(string[] args) {
        ...
        try {
            ...
            var Out = Console.Out; var MD = new StringBuilder();

            Console.SetOut(new StringWriter(MD));
            ...
            File.WriteAllText(@"lewis.md", MD.ToString());
